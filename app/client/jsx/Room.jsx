...
class Room extends Component {
  ...
  getRoomContent() {
    ...
    return {
      ...
      ADVENTURE: (
        <Adventure
          options={roomData}
          onClick={this.onAdventureClick.bind(this)}
        ></Adventure>
      ),
      IMAGEMAP: (
        <ImageMapRoom imageMapOptions={roomData.imageMapOptions}></ImageMapRoom>
      ),
    }[roomData.type];
  }
  ...
  computeMapState(room) {
    ...
    const visitedRooms = Object.values(this.props.visited).filter((x) => x)
      .length;
    const isMapUnlocked =
      Config.debug ||
      mapAlreadyUnlocked ||
      (_.has(this.roomTypesWithMap, room.type) &&
        visitedRooms >= mapUnlockThreshold);
  ...
  render() {
    ...
    const content = this.state.entered ? (
      ...
      <Door
        room={room}
        users={userList}
        tintColor={room.doorTint}
        onClick={this.onEnterRoom.bind(this)}
      ></Door>
    );
